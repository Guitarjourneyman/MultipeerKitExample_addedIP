...
public final class MultipeerTransceiver {
    ...
    /// Called on the main queue when a new peer discovered. -> Peer추가
    public var peerAdded: (Peer) -> Void = { _ in }

    /// Called on the main queue when a peer removed.
    public var peerRemoved: (Peer) -> Void = { _ in }

    /// Called on the main queue when a connection is established with a peer. -> 그룹 생성
    public var peerConnected: (Peer) -> Void = { _ in }
    
    /// Called on the main queue when the connection with a peer is interrupted. -> 그룹 해제
    ...
    public func getLocalIPAddress() -> String? {
        ...
        if getifaddrs(&ifaddr) == 0 {
            ...
            while ptr != nil {
                ...
                guard let addrFamily = ptr?.pointee.ifa_addr.pointee.sa_family else { continue }
                ...
                if addrFamily == AF_INET || addrFamily == AF_INET6 {
                    ...
                    if flags & IFF_RUNNING != 0 {
                        ...
                        if name == "en0" { // Wi-Fi interface
                            ...
                            if getnameinfo(ptr!.pointee.ifa_addr, socklen_t(ptr!.pointee.ifa_addr.pointee.sa_len), &hostname, socklen_t(hostname.count), nil, 0, NI_NUMERICHOST) == 0 {
    ...
    /// - Parameter payload: The payload to be sent. : saved message here
    public func broadcast<T: Encodable>(_ payload: T) {
        MultipeerMessage.register(T.self, for: String(describing: T.self))

        do {
            let senderIP = getLocalIPAddress() // Get the local IP address
            let message = MultipeerMessage(type: String(describing: T.self), payload: payload, senderIP: senderIP) // Pass senderIP to the initializer
